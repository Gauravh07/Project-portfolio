...
void eval(char *cmdline) 
{
    ...
    if (!builtin_cmd(argv)) {
        ...
        if ((pid = fork()) == 0) {
            ...
            signal(SIGINT, SIG_DFL);     // Restore default signal handling only for fork since it has stdout permissions now
        ...
        if (!bg) {
            ...
            tcsetpgrp(STDIN_FILENO, pid);  // Give control of stdin to the child process group
            sigprocmask(SIG_SETMASK, &prev_one, NULL);  
            waitfg(pid);
...
void sigint_handler(int sig) 
{
    ...
    if (pid != 0) {
        ...
        // Remove job from job list and update state
        struct job_t *job = getjobpid(jobs, pid);
        if (job != NULL) {
            deletejob(jobs, pid);
        }
