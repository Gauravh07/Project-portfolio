...
void* mm_malloc (size_t size) {
  ...
  size += 2*WORD_SIZE;
  // making sure the requested size meets min. block requirement
  if (size <= MIN_BLOCK_SIZE) {
    reqSize = MIN_BLOCK_SIZE;
  } else {
    // align the requested size to the nearest multiple of the alignment
    reqSize = ALIGNMENT * ((size + ALIGNMENT - 1) / ALIGNMENT);
  }

  // search for a free block in the free list that can satisfy the requested size
  ptrFreeBlock = searchFreeList(reqSize);
  // if no suitable free block is found, request more space
  if (ptrFreeBlock == NULL) {
    requestMoreSpace(reqSize);  
    ptrFreeBlock = searchFreeList(reqSize);// search free list after requesting more space
    if (ptrFreeBlock == NULL) { // return null if allocation failed
      return NULL;
    }
  }

  blockSize = SIZE(ptrFreeBlock->sizeAndTags);  // get block size
  size_t remSpace = blockSize - reqSize;  // setting up a variable for ease
  if (remSpace >= MIN_BLOCK_SIZE) {
    // splitting to fit another block
    BlockInfo * splitBlock = (BlockInfo*)UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize);
    // setting size and tag
    splitBlock->sizeAndTags = (remSpace) | TAG_PRECEDING_USED;
    // updating footer
    *((size_t*)UNSCALED_POINTER_ADD(splitBlock, remSpace - WORD_SIZE)) = splitBlock->sizeAndTags;
    // update size and tag
    ptrFreeBlock->sizeAndTags = reqSize | (ptrFreeBlock->sizeAndTags & TAG_PRECEDING_USED);
    // inserting the split block
    insertFreeBlock(splitBlock); 
  }

  // mark the block as used by setting the TAG_USED flag
  ptrFreeBlock->sizeAndTags |= TAG_USED;

  // calculate and set the footer to match the updated header tags
  size_t* blockFooter = (size_t*)UNSCALED_POINTER_ADD(ptrFreeBlock, SIZE(ptrFreeBlock->sizeAndTags) - WORD_SIZE);
  *blockFooter = ptrFreeBlock->sizeAndTags;

  // remove the block from the free list now that it’s allocated
  removeFreeBlock(ptrFreeBlock);

  // updating the allocated block from th free list since we are using it again
  ...
  return UNSCALED_POINTER_ADD(ptrFreeBlock, WORD_SIZE); // return the pointer to the allocated space
}
...
void mm_free (void *ptr) {
  ...
  // calculate block header location
  blockInfo = (BlockInfo*)UNSCALED_POINTER_SUB(ptr, WORD_SIZE);

  // mark the block as free
  blockInfo->sizeAndTags &= ~TAG_USED;

  // update the blocks following it
  followingBlock = (BlockInfo*)UNSCALED_POINTER_ADD(blockInfo, SIZE(blockInfo->sizeAndTags));
  
  // update the blocks header to indicate previous block is free
  followingBlock->sizeAndTags &= ~TAG_PRECEDING_USED;

  // calling insertFreeBlock to add the block to the list
  insertFreeBlock(blockInfo);

  // calling coalesce to merge blocks
