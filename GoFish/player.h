 ...
 *  Returns after finding one matching set of 4, so should be called after
 * adding each a new card.
 *
 *  target: pointer to the player to check
 *
 *  Return: a char that indicates the book that was added; return 0 if no book
 * added.
 */
 ...
 *   requested rank.  If not, print out "Error - must have at least one card
 * from rank to play" and then re-prompt the user.
 *
...
void remove_all_occurance_of_rank(struct player *p1, struct player *p2, char rank);
